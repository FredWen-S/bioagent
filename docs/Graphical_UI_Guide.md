...
确认后状态变为 `completed`，后端保存预演确认和任务指纹。真实执行只接受与当前任务匹配的
已确认预演；修改任务内容后必须重新预演和确认。确认预演不代表真实 BioRender 已验收。
...
3. 确认当前任务的安全预演结果。
4. 选择“真实执行”。
5. 勾选“我已确认使用可丢弃的空白 Figure”。
6. 先单击“仅校准界面”。校准会打开页面并检查普通搜索区、结果区和画布。
7. 校准证据正常后，才单击“开始绘制”。
...
读取；当前进程内仍存活的后台 Job ID、busy 状态和轮询会从状态 API 恢复，不会因为刷新而
重复启动。服务重启后，数据库中残留的执行态会转为 `paused_reconciliation`，供用户检查后
恢复，不会永久显示为正在运行。
...
暂停，不会粗暴杀死进程或绕过 SQLite Checkpoint。按钮优先停止当前 Job，因此人工登录等
没有 Figure ID 的浏览器任务也可安全关闭。线程已退出但内存状态未终结的 Job 会被自动
回收，不再永久占用浏览器锁。
